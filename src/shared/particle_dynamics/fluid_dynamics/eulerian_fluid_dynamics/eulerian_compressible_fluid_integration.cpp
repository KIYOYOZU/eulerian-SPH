...
void EulerianCompressibleIntegration1stHalfMUSCL<Inner<>>::interaction(size_t index_i, Real dt)
{
    ...
    for (size_t n = 0; n != inner_neighborhood.current_size_; ++n)
    {
        ...
        // The periodic cell-linked list retains the real neighbour index while
        // storing its translated image position. e_ij/r_ij carry that image
        // geometry, whereas pos_[index_j] remains the unwrapped real position.
        // Reconstruct from the neighbour image so MUSCL sees the same short
        // displacement as the flux and gradient operators.
...
void EulerianCompressibleIntegration2ndHalfMUSCL<Inner<>>::interaction(size_t index_i, Real dt)
{
    ...
    for (size_t n = 0; n != inner_neighborhood.current_size_; ++n)
    {
        ...
        const Vecd &xi = pos_[index_i];
        // Keep the reconstruction geometry consistent with the periodic
        // neighbour image represented by r_ij/e_ij; see the first-half pass.
        const Vecd xj = xi - inner_neighborhood.r_ij_[n] * e_ij;
        Vecd xf = 0.5 * (xi + xj);
...
void EulerianCompressibleIntegration1stHalfMUSCL<Contact<Wall>>::interaction(size_t index_i, Real dt)
{
    ...
    // this wall pass starts from zero and only appends wall flux, so the prior is
    // counted exactly once. Composed with Inner<> via the WithWall alias; used
    // standalone it applies no prior, and it has no update() to advance state.
...
void EulerianCompressibleIntegration2ndHalfMUSCL<Contact<Wall>>::interaction(size_t index_i, Real dt)
{
    ...
    CompressibleFluidState state_i(this->rho_[index_i], this->vel_[index_i], this->p_[index_i], energy_per_volume_i);
    Real mass_change_rate = 0.0;
    // Prior ownership contract: Inner<> seeds dE_dt_ with the prior work
    // ForcePrior dot Velocity and assigns; this wall pass starts from zero and
    // only appends wall flux. ForcePrior never enters continuity in either pass.
