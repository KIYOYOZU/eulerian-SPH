...

{
    ...
    {
        // No baseline provided for this run: skip the regression test instead
        // of aborting. resultTest/testNewResult already gate on number_of_run_
        // being > 1 and on a present Mean_Element file, so leaving the
        // member defaults (converged_ = "false", number_of_run_ = 1) is safe
        // and effectively disables the regression comparison for this case.
        std::cout << "[regression] " << runtimes_filefullpath_
                  << " not found; regression comparison disabled for this run." << std::endl;
        return;
    }
    else
    {
...
void RegressionTestBase<ObserveMethodType>::writeDataToXmlMemory(
    XmlEngine &xml_engine, XmlElement &element, const BiVector<T> &quantity,
    int snapshot, int observation, const std::string &quantity_name, StdVec<std::string> &element_tag)
{
    for (int l = 0; l != snapshot; ++l)
    {
        ...
        {
            XmlEngine::element_iterator ele_ite = element.element_begin(element_name);
...
template <typename T>
void RegressionTestBase<ObserveMethodType>::writeDataToXmlMemory(
    XmlEngine &xml_engine, XmlElement &element,
...
{
    int l = 0;
    XmlEngine::element_iterator ele_ite = element.element_begin();
...
void RegressionTestBase<ObserveMethodType>::readTagFromXmlMemory(
    XmlElement &element, StdVec<std::string> &element_tag)
{
    size_t l = 0;
    XmlEngine::element_iterator ele_ite = element.element_begin();
    for (; ele_ite != element.element_end(); ++ele_ite)
    {
        element_tag[l] = std::string(ele_ite->Name());
        l++;
    }
}
//=================================================================================================//
...
void RegressionTestBase<ObserveMethodType>::readResultFromXml()
{
    if (number_of_run_ > 1) /*only read the result from the 2nd run, because the 1st run doesn't have previous results. */
    {
        ...
        for (int run_index = 0; run_index != number_of_run_ - 1; ++run_index)
        {
            ...
            XmlElement father_element = result_xml_engine_in_.getChildElement(node_name);
            for (int k = 0; k != observation_; ++k)
...
void RegressionTestBase<ObserveMethodType>::readResultFromXml(int run_index)
{
    if (number_of_run_ > 1) /*only read the result from the 2nd run, because the 1st run doesn't have previous results. */
    {
        ...
        XmlElement snapshot_element = result_xml_engine_in_.getChildElement("Snapshot_Element");
        XmlEngine::element_iterator ele_ite = snapshot_element.element_begin();
        ...
        XmlElement result_element = result_xml_engine_in_.getChildElement("Result_Element");
        for (int l = 0; l != snapshot_; ++l)
        {
            int k = 0;
            XmlEngine::element_iterator ele_ite = result_element.element_begin();
...
void RegressionTestBase<ObserveMethodType>::writeResultToXml(int run_index)
{
    ...
    XmlElement snapshot_element = result_xml_engine_out_.getChildElement("Snapshot_Element");
    ...
    XmlElement result_element = result_xml_engine_out_.getChildElement("Result_Element");
    for (int k = 0; k != observation_; ++k)
    {
        ...
        for (int l = 0; l != snapshot_; ++l)
        {
            XmlEngine::element_iterator ele_ite = result_element.element_begin(element_name);
            std::string attribute_name = "snapshot_" + std::to_string(l);
