...
# -*- coding: utf-8 -*-
"""
Compute spanwise-normalized Cd/Cl for the 3D Eulerian LG cylinder case.
...
``0.5 * rho0 * u_f^2 * D * DW`` (D = cylinder diameter, DW = spanwise
periodic width). The force files share the simulation time column, so
rows are aligned position-wise and a small mismatch (<= 1e-10 s) is
silently snapped to the minimum of the two.
...
def main() -> int:
    ...
    parser.add_argument("--rho0", type=float, required=True, help="reference density")
    parser.add_argument("--u-ref", type=float, required=True, help="freestream velocity")
    parser.add_argument("--diameter", type=float, required=True, help="cylinder diameter")
    parser.add_argument("--span-width", type=float, required=True, help="periodic span width DW")
    parser.add_argument("--out", type=Path, default=Path("output") / "cylinder_3d_cl_cd.csv", help="output CSV path")
    args = parser.parse_args()
    compute(args.output_dir, args.rho0, args.u_ref, args.diameter, args.span_width, args.out)
