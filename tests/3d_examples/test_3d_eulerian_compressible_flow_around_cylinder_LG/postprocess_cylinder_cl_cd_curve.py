...
Plot Cd(t) / Cl(t) time-series curves for the 3D Eulerian fully compressible cylinder case.
...
def plot_cl_cd_combined(
    ...
    fig.suptitle("3D Eulerian Compressible Cylinder -- Cd / Cl time history", y=0.985)
    ...
    if cfg is not None:
        info = (
            f"$Ma_\\infty$ = {cfg.get('mach_inf', float('nan')):.2f},  "
            f"$Re$ = {cfg.get('re', float('nan')):.0f},  "
            f"$\\rho_\\infty$ = {cfg.get('rho_inf', float('nan')):.3f},  "
            f"$u_\\infty$ = {cfg.get('u_inf', float('nan')):.3f},  "
            f"$p_\\infty$ = {cfg.get('p_inf', float('nan')):.4f},  "
...
def plot_force_components(
    ...
    time = data["time"]
    # Require the keys instead of substituting 1.0: a silent fallback would yield
    # plausible-looking but wrong coefficients from a broken config.
    context = cfg or {}
    required = ("rho_inf", "u_inf", "D", "DW")
    missing = [key for key in required if key not in context]
    if missing:
        raise ValueError(
            f"cannot normalise force components, missing {missing}; config.ini must supply "
            "gamma / rho_inf / c_inf / mach_inf / cylinder_radius / dw"
        )
    # Same normalisation as postprocess_cl_cd.py: 0.5*rho_inf*u_inf^2*D*DW.
    scale_denom = (
        0.5
        * float(context["rho_inf"])
        * float(context["u_inf"]) ** 2
        * float(context["D"])
        * float(context["DW"])
    )
    if not math.isfinite(scale_denom) or scale_denom <= 0.0:
        raise ValueError(f"invalid Cd/Cl normalisation scale {scale_denom}; check config.ini")
    ...
    fig.suptitle("3D Eulerian Compressible Cylinder -- Drag / Lift components", y=0.985)
...
def load_case_context(config_path: Path) -> Dict[str, float]:
    """Read the case context for plot annotations and Cd/Cl normalisation.

    ``u_inf`` is derived as ``mach_inf * c_inf``, matching the C++ config, so the
    annotation and the normalisation can never drift from the solver.
    """
    ...
    info: Dict[str, float] = {}
    try:
        gamma = float(parser["physical"]["gamma"])
        rho_inf = float(parser["physical"]["rho_inf"])
        c_inf = float(parser["physical"]["c_inf"])
        mach_inf = float(parser["physical"]["mach_inf"])
        info["gamma"] = gamma
        info["rho_inf"] = rho_inf
        info["c_inf"] = c_inf
        info["mach_inf"] = mach_inf
        info["u_inf"] = mach_inf * c_inf
        info["p_inf"] = rho_inf * c_inf * c_inf / gamma
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Plot Cd/Cl time-series for the 3D Eulerian fully compressible cylinder case.",
    )
